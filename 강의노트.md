...
서비스 주소를 쳐서 요청을 보낸 후,
npm start - 개발모드라는 경고창이 사라졌다. headers에 Content endcoding이 gzip으로 변경된다.

gzip처리가 안되었다면, 용량이 엄청 뻥튀기 된다.
검색엔진이 보는 눈이라고 생각하면 된다.

header, footer, article, h1, h2, 등등

CSS 문서가 너무 길면 extractCSS를 사용하자 - nuxt 영어 문서

nuxt.config.js파일에 내용 추가한다.

6:25~ og태그 추가 (open graph) - facebook에서 만듦

og: 공유했을때 뜬 이미지
og:url - 검색엔진, SNS 서비스

SEND 해본다. 배포모드는 코드변경될때마다 다시 빌드해야한다.

개별 페이지마다 head 추가해줘야한다.
head() 개별설정. 기본적으로 nuxt.config.js에 설정으로 들어가고 덮어씌워짐
